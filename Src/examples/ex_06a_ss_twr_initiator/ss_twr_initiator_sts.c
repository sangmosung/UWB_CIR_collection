...
static uint8_t cir_buffer[6097];
static int length = 6097;
static int offset = 0;
/* Hold the amount of errors that have occurred */
static uint32_t errors[23] = {0};
static float CIRValues[1016] = {0};
//float *CIRValues2;
//static int32_t CIR[1016];
int cx = 0;
int cx_temp = 0;
...
//        // CIR
//        dwt_readaccdata(cir_buffer, length, offset);
//
//        int byteAddress = 1;
//
...
//
//        }
//        test_run_info("Hello World!\r\n");
//        for (int j=0; j<1016; j++){
//
//        	snprintf(CIRValues, sizeof(CIRValues),"%.2f, ", CIRValues[j]);
//        	test_run_info(CIRValues);
//        	Sleep(0.001);
//        	if (j == 1015)
//        	{
//        		test_run_info("\r\n");
            ...
            if (frame_len <= sizeof(rx_buffer))
            {
                ...
                if (memcmp(rx_buffer, rx_resp_msg, ALL_MSG_COMMON_LEN) == 0)
                {
                    ...
                    for (int i=0; i<1016; i++) {
                        ...
                        if (qValue & 0x020000)  // MSB of 18 bit value is 1
                            qValue |= 0xfffc0000;
                        *(CIRValues+i) = sqrt((float)(iValue*iValue+qValue*qValue)); // float형
					...
					for (int j=0; j<1016; j++){
						...
						cx = snprintf(buffer+cx_temp, bufsize-cx_temp, "%.2f,", CIRValues[j]);
						cx_temp = cx_temp + cx;

						if (j == 1015)
						{
							cx = snprintf(buffer+cx_temp, bufsize-cx_temp , "%.2f", distance);
//							test_run_info(buffer);
//							test_run_info("\r\n");
							cx_temp = cx_temp + cx;
							HAL_UART_Transmit(&huart3, buffer, cx_temp, 2000);
 ...
 *    6.8M data rate used (around 200 탎).
